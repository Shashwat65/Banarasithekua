...
const fetchProduct = async (slug: string) => {
  // Use the backend endpoint that returns a single product by slug
  const res = await fetch(`/api/products/slug/${encodeURIComponent(slug)}`);
  if (res.status === 404) return null;
  if (!res.ok) throw new Error("Failed to load product");
  const data = await res.json();
  // backend sends { success: true, data: product }
  return data.data;
};
...
export default function ProductDetail() {
  ...
  const product = useMemo(() => {
    // Prefer API response when available, otherwise fall back to sample products
    if (data) return data;
    if (slug) return findSampleProduct(slug);
    return null;
  }, [data, slug]);
